        ...
        if self.subagents.is_root_thread(thread_id) {
            match &event.msg {
                ...
                EventMsg::CollabWaitingBegin(ev) => {
                    self.emit_or_queue_subagent_history(Box::new(multi_agents::waiting_begin(
                        ev.clone(),
                    )));
                }
                EventMsg::CollabWaitingEnd(ev) => {
                    self.subagents.on_wait_end(ev);
                    self.emit_or_queue_subagent_history(Box::new(multi_agents::waiting_end(
                        ev.clone(),
                    )));
                }
                EventMsg::CollabCloseEnd(ev) => {
                    self.emit_or_queue_subagent_history(Box::new(multi_agents::close_end(
                        ev.clone(),
                    )));
