...
const PLAN_IMPLEMENTATION_YES: &str = "Yes, implement in Execute mode";
...
pub(super) const PLAN_IMPLEMENTATION_EXECUTE_UNAVAILABLE: &str = "Execute mode unavailable";
...
pub(super) fn selection_view_params(
    execute_mask: Option<CollaborationModeMask>,
    plan_markdown: Option<&str>,
    clear_context_usage_label: Option<&str>,
) -> SelectionViewParams {
    let (implement_actions, implement_disabled_reason) = match execute_mask.clone() {
        Some(mask) => {
    ...
    let (clear_context_actions, clear_context_disabled_reason) = match (execute_mask, plan_markdown)
    {
        (None, _) => (
            Vec::new(),
            Some(PLAN_IMPLEMENTATION_EXECUTE_UNAVAILABLE.to_string()),
