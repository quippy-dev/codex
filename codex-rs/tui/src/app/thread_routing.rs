...
impl App {
    ...
    pub(super) async fn infer_session_for_thread_notification(
        ...
        if let Some(model) = read_session_model(None, thread_id, rollout_path.as_deref()).await {
            session.model = model;
    ...
    pub(super) fn handle_skills_list_response(&mut self, response: SkillsListResponse) {
        ...
        let core_response = list_skills_response_to_core(response.clone());
        let errors = errors_for_cwd(&cwd, &core_response);
