...
#[test]
fn exec_ephemeral_fork_does_not_persist_child_rollout() -> anyhow::Result<()> {
    let test = test_codex_exec();
    let fixture = exec_fixture()?;
    ...
    let marker2 = format!("fork-ephemeral-{}", Uuid::new_v4());
    let prompt2 = format!("echo {marker2}");
    ...
    assert!(
        find_session_file_containing_marker(&sessions_dir, &marker2).is_none(),
        "ephemeral fork should not persist a child rollout"
    );
