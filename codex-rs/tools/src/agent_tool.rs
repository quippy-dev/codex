...
pub fn create_send_input_tool_v1() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target".to_string(),
            JsonSchema::string(Some(
                "Agent id to message (from spawn_agent). Omit or use parent/root from a subagent to target its parent."
                    .to_string(),
            )),
        ),
        (
            "id".to_string(),
            JsonSchema::string(Some(
                "Compatibility alias for target. Omit or use parent/root from a subagent to target its parent."
                    .to_string(),
            )),
        ),
        (
...
pub fn create_peek_agents_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "id".to_string(),
            JsonSchema::string(Some(
                "Optional owner agent id, or parent/root/self alias. Defaults to the current agent."
                    .to_string(),
            )),
        ),
        (
            "recursive".to_string(),
            JsonSchema::boolean(Some(
                "When true (default), include descendant agents recursively.".to_string(),
            )),
        ),
        (
            "cursor".to_string(),
            JsonSchema::number(Some(
                "Optional progress cursor. When provided, only agents with newer progress are returned."
                    .to_string(),
            )),
        ),
        (
            "limit".to_string(),
            JsonSchema::number(Some(
                "Maximum number of changed agents to return.".to_string(),
            )),
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Return pull-only incremental progress snapshots for live agents without waiting or delivering queued messages."
            .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::object(properties, /*required*/ None, Some(false.into())),
        output_schema: Some(peek_agents_output_schema()),
    })
}
...
pub fn create_compact_parent_context_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "reason".to_string(),
            JsonSchema::string(Some(
                "Optional reason for requesting parent context compaction.".to_string(),
            )),
        ),
        (
            "evidence".to_string(),
            JsonSchema::string(Some(
                "Optional short evidence summary for the compaction request.".to_string(),
            )),
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Request compaction of the parent context from an active watchdog helper."
            .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::object(properties, /*required*/ None, Some(false.into())),
        output_schema: Some(compact_parent_context_output_schema()),
    })
}
...
pub fn create_list_agents_tool_v1() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "id".to_string(),
            JsonSchema::string(Some(
        ...
        (
            "all".to_string(),
            JsonSchema::boolean(Some(
                "When true, list all tracked live agents in this root tree.".to_string(),
            )),
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "List live agents by id, parent id, status, and depth.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::object(properties, /*required*/ None, Some(false.into())),
        output_schema: Some(list_agents_output_schema_v1()),
    })
}
            ...
            "agents": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "spawn_mode": {
                            "type": ["string", "null"],
                            "enum": ["spawn", "fork", "watchdog", null],
                            "description": "Watchdog when the listed agent is a watchdog handle; otherwise null."
                        }
                    },
                    "required": ["agent_name", "agent_status", "last_task_message", "spawn_mode"],
                    "additionalProperties": false
                },
...
fn list_agents_output_schema_v1() -> Value {
    json!({
        "type": "object",
        "properties": {
            "agents": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "depth": { "type": "integer" },
                        "spawn_mode": { "type": ["string", "null"], "enum": ["spawn", "fork", "watchdog", null] }
                    },
                    "required": ["id", "parent_id", "status", "depth", "spawn_mode"],
                    "additionalProperties": false
                }
            }
        },
...
fn peek_agents_output_schema() -> Value {
    json!({
        "type": "object",
        "properties": {
            "agents": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "latest_message_preview": { "type": ["string", "null"] },
                        "terminal_summary": { "type": ["string", "null"] }
                    },
                    "required": [
                        "id",
                        "parent_id",
...
fn compact_parent_context_output_schema() -> Value {
    json!({
        "type": "object",
        "properties": {
            "parent_id": { "type": "string" },
        ...
        "required": ["parent_id", "submission_id"],
        "additionalProperties": false
    })
}
...
fn spawn_agent_common_properties_v1(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            ...
            JsonSchema::string(Some(
                "Canonical spawn mode: spawn, fork, or watchdog. fork_context=true is treated as fork unless spawn_mode=watchdog, which is invalid."
                    .to_string(),
            )),
        ),
        (
            ...
            JsonSchema::number(Some(
                "Watchdog interval in seconds when spawn_mode=watchdog.".to_string(),
            )),
        ),
        (
            "model".to_string(),
...
fn spawn_agent_common_properties_v2(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            "spawn_mode".to_string(),
            JsonSchema::string(Some(
                "Canonical spawn mode: spawn, fork, or watchdog. When omitted, fork_turns controls spawn vs fork."
                    .to_string(),
            )),
        ),
        (
