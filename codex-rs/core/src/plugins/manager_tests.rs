...
#[tokio::test]
async fn plugins_for_config_reapplies_runtime_product_restriction() {
    let codex_home = TempDir::new().unwrap();
  ...
  "plugins": [
    ...
    assert_eq!(
        outcome.effective_skill_roots(),
        vec![codex_plugin_root.join("skills").abs()]
    );
    assert_eq!(
        outcome.effective_apps(),
...
#[tokio::test]
async fn plugins_for_config_keeps_runtime_capabilities_when_restriction_product_is_none() {
    let codex_home = TempDir::new().unwrap();
    let curated_root = curated_plugins_repo_path(codex_home.path());
...
#[tokio::test]
async fn plugins_for_config_cache_is_keyed_by_config_layer_stack() {
    let codex_home = TempDir::new().unwrap();
    ...
    let sample_config = load_config(codex_home.path(), codex_home.path()).await;
    let sample_outcome = manager.plugins_for_config(&sample_config).await;
    assert_eq!(
        sample_outcome
            .plugins()
            .iter()
    ...
    let other_config = load_config(codex_home.path(), codex_home.path()).await;
    let other_outcome = manager.plugins_for_config(&other_config).await;
    assert_eq!(
        other_outcome
            .plugins()
            .iter()
