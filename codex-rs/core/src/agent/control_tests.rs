...
async fn wait_for_active_turn(thread: &Arc<CodexThread>) -> bool {
    let wait = async {
        loop {
...
async fn install_blocking_startup_prewarm(thread: &Arc<CodexThread>) {
    let handle = tokio::spawn(async move {
        ...
        Ok(crate::client::ModelClient::new(
            None,
...
async fn root_watchdog_helper_send_input_to_idle_owner_wakes_owner_without_terminal_completion_chatter()
 {
    ...
    let helper_turn = helper_thread
        .codex
        .session
        .new_default_turn_with_sub_id("idle-owner-watchdog-helper-turn".to_string())
        .await;
    helper_thread
        .codex
        .session
    ...
    helper_thread
        ...
        .on_task_finished(
            Arc::clone(&helper_turn),
            Some("Reported completion to parent.".to_string()),
        )
        .await;
    ...
    assert!(
        !wait_for_raw_response_text_with_timeout(
            &owner_thread,
            "Reported completion to parent.",
            std::time::Duration::from_millis(300),
        )
        .await,
        "watchdog helper completion text should be suppressed after successful send_input"
    );
    assert!(
        !wait_for_raw_response_text_with_timeout(
            &owner_thread,
        ...
        .await,
        "watchdog helper should not emit synthetic fallback after successful send_input"
    );
...
#[tokio::test]
async fn close_agent_marks_persisted_subtree_closed_when_target_is_offline() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    ...
    state_db
        .set_thread_spawn_edge_status(child_thread_id, DirectionalThreadSpawnEdgeStatus::Open)
        .await
        .expect("child edge reopen should succeed");
    state_db
        .set_thread_spawn_edge_status(grandchild_thread_id, DirectionalThreadSpawnEdgeStatus::Open)
        .await
        .expect("grandchild edge reopen should succeed");
    let open_descendants_before = state_db
        .list_thread_spawn_descendants_with_status(
            child_thread_id,
            DirectionalThreadSpawnEdgeStatus::Open,
        )
        .await
        .expect("open descendants from child should load");
    assert_eq!(open_descendants_before, vec![grandchild_thread_id]);

    let close_offline_result = harness.control.close_agent(child_thread_id).await;
    assert_matches!(
        close_offline_result,
        Err(CodexErr::ThreadNotFound(id)) if id == child_thread_id
    );
    ...
    let closed_children = state_db
        ...
        .await
        .expect("closed children from parent should load");
    ...
    let closed_descendants = state_db
        ...
        .await
        .expect("closed descendants from child should load");
    assert_eq!(closed_descendants, vec![grandchild_thread_id]);
}
