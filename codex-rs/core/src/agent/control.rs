...
use crate::agent::progress_cache::AgentProgressCache;
use crate::agent::progress_cache::AgentProgressSnapshot;
use crate::agent::registry::AgentMetadata;
use crate::agent::registry::AgentRegistry;
use crate::agent::role::DEFAULT_ROLE_NAME;
use crate::agent::role::resolve_role_config;
use crate::agent::status::is_final;
use crate::agent::watchdog::RemovedWatchdog;
use crate::agent::watchdog::WatchdogManager;
use crate::agent::watchdog::WatchdogRegistration;
...
pub(crate) struct ListedAgent {
    ...
    pub(crate) spawn_mode: Option<AgentSpawnMode>,
}
...
impl Default for AgentControl {
    fn default() -> Self {
        ...
        Self {
            ...
            watchdogs: WatchdogManager::new(manager),
...
impl AgentControl {
    ...
    pub(crate) async fn compact_parent_for_watchdog_helper(
        ...
        if agent_status_is_active(&parent_thread.agent_status().await) {
    ...
    /// Spawn a watchdog handle thread without submitting an initial prompt.
    ///
    /// Watchdog handles are user-addressable lifecycle targets. The watchdog manager forks
    /// short-lived helper agents from the owner when the owner has been idle long enough.
    pub(crate) async fn spawn_agent_handle(
        ...
        if options.fork_mode.is_some() {
            return Err(CodexErr::Fatal(
                "watchdog handle spawn must not request fork mode".to_string(),
            ));
        }
        let state = self.upgrade()?;
    ...
    pub(crate) async fn send_watchdog_wakeup(
        ...
        let recipient = owner_thread
            .config_snapshot()
            .await
            .session_source
            .get_agent_path()
            .unwrap_or_else(AgentPath::root);
        let author = if let Some(agent_path) = self
            .get_agent_metadata(helper_thread_id)
            .and_then(|metadata| metadata.agent_path)
        {
            agent_path
        } else if let Ok(helper_thread) = state.get_thread(helper_thread_id).await {
            helper_thread
                .config_snapshot()
                .await
                .session_source
                .get_agent_path()
                .unwrap_or_else(|| fallback_watchdog_helper_path(&recipient, helper_thread_id))
        } else {
            fallback_watchdog_helper_path(&recipient, helper_thread_id)
        };
        ...
        let result = self
            .send_inter_agent_communication(owner_thread_id, communication)
            .await;
        if result.is_ok() {
            self.mark_watchdog_helper_notified_owner_if_match(helper_thread_id, owner_thread_id)
                .await;
        }
        result
    }
    ...
    pub(crate) async fn run_watchdogs_once_for_tests(&self) {
        self.watchdogs.run_once(self).await;
    }
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> CodexResult<String> {
        let state = self.upgrade()?;
        for removed_watchdog in self.watchdogs.take_for_owner(agent_id).await {
            let _ = self
                .shutdown_live_agent(removed_watchdog.target_thread_id)
                .await;
    ...
    pub(crate) async fn get_agent_nickname_and_role(
        ...
        if let Some(metadata) = self.state.agent_metadata_for_thread(agent_id) {
            return Some((metadata.agent_nickname, metadata.agent_role));
        }
        let Ok(state) = self.upgrade() else {
            return None;
        };
    ...
    pub(crate) async fn list_agents(
        ...
        let live_agent_thread_ids = live_agents
            .iter()
            .filter_map(|metadata| metadata.agent_id)
            .collect::<Vec<_>>();
        let watchdog_target_ids = self.watchdog_targets(&live_agent_thread_ids).await;
    ...
    pub(crate) async fn list_agent_status_entries(
        &self,
        owner_thread_id: ThreadId,
        recursive: bool,
        all: bool,
        ...
        let mut status_by_thread: HashMap<ThreadId, AgentStatus> =
            HashMap::with_capacity(thread_ids.len());
        let mut depth_by_thread: HashMap<ThreadId, usize> =
            HashMap::with_capacity(thread_ids.len());
...
fn fallback_watchdog_helper_path(owner_path: &AgentPath, helper_thread_id: ThreadId) -> AgentPath {
    let safe_id = helper_thread_id
        .to_string()
        .chars()
        .map(|ch| {
