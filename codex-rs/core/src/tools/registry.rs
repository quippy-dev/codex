...
fn unsupported_tool_call_message(
    ...
    let display_tool_name = tool_handler_key(tool_name, namespace);
    let message = match payload {
        ToolPayload::Custom { .. } => {
            format!("unsupported custom tool call: {display_tool_name}")
        }
        _ => format!("unsupported call: {display_tool_name}"),
    };
