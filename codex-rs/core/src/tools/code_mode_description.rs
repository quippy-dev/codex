        ...
        ToolSpec::Function(mut tool) => {
            if tool.name != PUBLIC_TOOL_NAME {
                tool.description = append_code_mode_sample(
                    ...
                    code_mode_output_type(&tool.name, tool.output_schema.as_ref()),
                );
            }
...
fn code_mode_output_type(tool_name: &str, output_schema: Option<&JsonValue>) -> String {
    output_schema
        .map(render_json_schema_to_typescript)
        .or_else(|| {
            split_qualified_tool_name(tool_name).is_some().then_some(
                render_json_schema_to_typescript(&default_mcp_result_schema()),
            )
        })
        .unwrap_or_else(|| "unknown".to_string())
}

fn default_mcp_result_schema() -> JsonValue {
    json!({
        "type": "object",
        "properties": {
            "content": {
...
#[cfg(test)]
mod tests {
    use super::code_mode_output_type;
    ...
    #[test]
    fn code_mode_output_type_defaults_mcp_tools_to_call_tool_result_shape() {
        assert_eq!(
            code_mode_output_type("mcp__rmcp__echo", None),
