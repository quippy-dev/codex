            ...
            SpawnMode::Spawn => {
                async {
                    let thread_id = agent_control
                        .spawn_agent_handle(config, Some(spawn_source.clone()))
                        .await?;
                    agent_control.send_input(thread_id, input_items).await?;
                    Ok(thread_id)
                }
                .await
            }
...
mod send_input {
    ...
    pub async fn handle(
        ...
        let send_to_parent = match args.id.as_deref().map(str::trim) {
            Some(id) if !id.is_empty() && !matches!(id, "parent" | "root") => false,
            _ => true,
        };
