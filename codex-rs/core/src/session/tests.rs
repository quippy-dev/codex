use super::turn::PlanModeStreamState;
use super::turn::maybe_complete_plan_item_from_message;
...
async fn later_plain_plan_mode_messages_do_not_clear_cached_latest_proposed_plan_text() {
    ...
    assert!(cached_plan_text.is_some());
...
async fn non_app_mcp_tools_remain_visible_without_search_selection() {
    ...
    let config = test_config().await;
    selected_mcp_tools.extend(crate::mcp_tool_exposure::filter_codex_apps_mcp_tools(
        &mcp_tools,
        &connectors,
        &config,
    ));
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let rollout_items = vec![RolloutItem::Compacted(CompactedItem {
        message: String::new(),
        retained_proposed_plan: codex_protocol::RetainedProposedPlan::None,
...
async fn pending_request_user_input_does_not_spawn_extra_goal_continuation() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        ...
        config
            ...
            .expect("request_user_input outside Plan mode should be enableable in tests");
    });
    let test = builder.build(&server).await?;
