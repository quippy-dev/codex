...
async fn spawn_agent_tool_description_mentions_role_locked_settings() -> Result<()> {
    ...
    let _custom_role_description =
        role_block(&agent_type_description, "custom").expect("custom role description");
    assert!(
        agent_type_description.contains("custom: {"),
        "missing custom role block: {agent_type_description:?}"
    );
    assert!(
        agent_type_description.contains("Default spawn mode: spawn"),
        "missing default spawn mode note: {agent_type_description:?}"
    );
    assert!(
        agent_type_description.contains(
            "- This role's model is set to `gpt-5.1-codex-max` and its reasoning effort is set to `high`. These settings cannot be changed."
        ),
        "missing locked-settings note: {agent_type_description:?}"
    );
