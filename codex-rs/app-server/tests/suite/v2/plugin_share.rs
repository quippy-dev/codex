...
#[tokio::test]
async fn plugin_share_list_paginates_created_workspace_plugins() -> Result<()> {
    let codex_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_remote_plugin_config(codex_home.path(), &format!("{}/backend-api", server.uri()))?;
    write_chatgpt_auth(
    ...
    Mock::given(method("GET"))
        ...
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "plugins": [remote_plugin_json("plugins_123"), remote_plugin_json("plugins_456")],
            "pagination": empty_pagination_json(),
        })))
        .expect(2)
        .mount(&server)
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "plugins": [
                installed_remote_plugin_json("plugins_123"),
                installed_remote_plugin_json("plugins_456"),
            ],
            "pagination": empty_pagination_json(),
        })))
        .expect(2)
    ...
    let request_id = mcp
        .send_raw_request("plugin/share/list", Some(json!({ "limit": 1 })))
        .await?;
    let response: JSONRPCResponse = timeout(
        DEFAULT_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(request_id)),
    ...
    assert_eq!(response.data[0].plugin.id, "plugins_123");
    let next_cursor = response.next_cursor.expect("first page should continue");
    ...
    assert_eq!(response.data.len(), 1);
    assert_eq!(response.data[0].plugin.id, "plugins_456");
