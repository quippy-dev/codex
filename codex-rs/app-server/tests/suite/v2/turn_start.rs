    ...
    assert!(
        developer_texts
            ...
            .any(|text| text.contains("<personality_spec>")
                || text.contains("# You are the Root Agent")),
        "expected personality update signal in developer input, got {developer_texts:?}"
    );
...
#[tokio::test]
async fn turn_start_hides_request_user_input_in_default_mode_without_flag_v2() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    create_config_toml(
        ...
        &server.uri(),
        "never",
        &BTreeMap::from([(Feature::RequestUserInputOutsidePlanMode, false)]),
    )?;
...
async fn turn_start_change_personality_mid_thread_v2() -> Result<()> {
    ...
    assert!(
        second_developer_texts
            ...
            .any(|text| text.contains("<personality_spec>")
                || text.contains("# You are the Root Agent")),
        "expected personality update signal in second request, got {second_developer_texts:?}"
    );
...
async fn turn_start_uses_migrated_pragmatic_personality_without_override_v2() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(LOCAL_PRAGMATIC_TEMPLATE)
            || instructions_text.contains("You are Codex, a coding agent based on GPT-5"),
        "expected startup-migrated pragmatic personality signal in model instructions, got: {instructions_text:?}"
    );
...
"#
    ...
    assert_eq!(id, SPAWN_CALL_ID);
    assert_eq!(tool, CollabAgentTool::SpawnAgent);
    assert_eq!(spawn_mode, Some(CollabAgentSpawnMode::Spawn));
    assert_eq!(status, CollabAgentToolCallStatus::Completed);
    assert_eq!(sender_thread_id, thread.id);
    assert_eq!(receiver_thread_ids, vec![receiver_thread_id.clone()]);
    let [receiver_agent] = receiver_agents.as_slice() else {
        panic!("spawn completion should include one receiver agent");
    };
    assert_eq!(receiver_agent.thread_id, receiver_thread_id);
    assert!(receiver_agent.agent_nickname.is_some());
    assert_eq!(receiver_agent.agent_role.as_deref(), Some("custom"));
    assert_eq!(receiver_agent.spawn_mode, Some(CollabAgentSpawnMode::Spawn));
    assert_eq!(prompt, Some(CHILD_PROMPT.to_string()));
    ...
    assert_eq!(agent_status.thread_id, receiver_thread_ids[0]);
