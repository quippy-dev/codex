...
use codex_app_server_protocol::CollabAgentSpawnMode as V2CollabAgentSpawnMode;
    ...
    match msg {
        ...
        EventMsg::CollabWaitingBegin(begin_event) => {
            let receiver_thread_ids = begin_event
                ...
                .collect();
            let receiver_agents = wait_receiver_agents(&begin_event);
            let item = ThreadItem::CollabAgentToolCall {
                id: begin_event.call_id,
...
fn wait_receiver_agents(
    ...
    if begin_event.receiver_agents.is_empty() {
        return begin_event
            .receiver_thread_ids
            .iter()
...
fn wait_end_agent_statuses(
    ...
    if end_event.agent_statuses.is_empty() {
        let mut sorted_entries: Vec<(String, V2CollabAgentStatus)> = end_event
            .statuses
            .iter()
            .map(|(id, status)| (id.to_string(), V2CollabAgentStatus::from(status.clone())))
            .collect();
...
mod tests {
    ...
    #[test]
    fn collab_spawn_end_maps_effective_model_and_effort() {
        ...
        let item = collab_spawn_end_item(event.clone());
        let expected = ThreadItem::CollabAgentToolCall {
            id: event.call_id,
            tool: CollabAgentTool::SpawnAgent,
    ...
    #[tokio::test]
    async fn collab_spawn_end_live_projection_uses_end_event_model_and_effort() -> Result<()> {
        ...
        let begin_event = CollabAgentSpawnBeginEvent {
            call_id: "call-spawn".to_string(),
            sender_thread_id: ThreadId::new(),
            prompt: "new task".to_string(),
            model: "gpt-5".to_string(),
        ...
        thread_state
            .lock()
            .await
            .track_current_turn_event(&EventMsg::CollabAgentSpawnBegin(begin_event));

        let end_event = CollabAgentSpawnEndEvent {
            call_id: "call-spawn".to_string(),
            sender_thread_id: ThreadId::new(),
        ...
        apply_bespoke_event_handling(
            Event {
                id: "turn-1".to_string(),
                msg: EventMsg::CollabAgentSpawnEnd(end_event.clone()),
            },
            conversation_id,
            conversation,
            thread_manager,
        ...
        match msg {
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(n)) => {
                ...
                assert_eq!(n.item, collab_spawn_end_item(end_event));
            }
