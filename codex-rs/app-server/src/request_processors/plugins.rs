...
impl PluginRequestProcessor {
    ...
    async fn plugin_share_list_response(
        ...
        let PluginShareListParams { cursor, limit } = params;
        let (config, auth) = self.load_plugin_share_config_and_auth().await?;
        let remote_plugin_service_config = RemotePluginServiceConfig {
            chatgpt_base_url: config.chatgpt_base_url.clone(),
        };
        let data: Vec<PluginShareListItem> = codex_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.as_path(),
        )
        ...
        if start > total {
            return Err(invalid_request(format!(
                "cursor {start} exceeds total plugin shares {total}"
            )));
        }
