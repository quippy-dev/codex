...
- `collabToolCall` — `{id, tool, status, senderThreadId, receiverThreadIds, receiverAgents, prompt?, spawnMode?, model?, reasoningEffort?, closeResult?, agentsStates, agentStatuses}` describing collab tool calls (`spawn_agent`, `send_input`, `resume_agent`, `wait`, `close_agent`); `status` is `inProgress`, `completed`, or `failed`.
