...
fn app_server_collab_agent_statuses_to_core(
    ...
    receiver_agents: &[codex_app_server_protocol::CollabAgentRef],
    ...
    for receiver_thread_id in receiver_thread_ids {
        ...
        agent_statuses.push(CollabAgentStatusEntry {
            ...
            spawn_mode: receiver_agent
                .and_then(|agent| agent.spawn_mode)
                .map(codex_app_server_protocol::CollabAgentSpawnMode::to_core),
            status: status.clone(),
        });
...
impl ChatWidget {
    ...
    fn open_plan_implementation_prompt(&mut self) {
        let execute_mask =
            collaboration_modes::mask_for_kind(self.model_catalog.as_ref(), ModeKind::Execute);
        ...
        let (default_actions, default_disabled_reason) = match default_mask {
            Some(mask) => {
                let user_text = PLAN_IMPLEMENTATION_CODING_MESSAGE.to_string();
                let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
                    tx.send(AppEvent::SubmitUserMessageWithMode {
    ...
    fn on_collab_agent_tool_call(&mut self, item: ThreadItem) {
        ...
        let first_receiver_agent = receiver_thread_ids.first().and_then(|thread_id| {
            receiver_agents
                .iter()
                    ...
                    self.on_collab_event(multi_agents::spawn_end(
                        codex_protocol::protocol::CollabAgentSpawnEndEvent {
                            ...
                            spawn_mode: spawn_mode
                                .map(codex_app_server_protocol::CollabAgentSpawnMode::to_core)
                                .unwrap_or(codex_protocol::protocol::AgentSpawnMode::Spawn),
                    ...
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            codex_protocol::protocol::CollabResumeBeginEvent {
                                ...
                                receiver_agent_role: first_receiver_agent
                                    .and_then(|agent| agent.agent_role.clone()),
                                receiver_spawn_mode: first_receiver_agent
                                    .and_then(|agent| agent.spawn_mode)
                                    .map(codex_app_server_protocol::CollabAgentSpawnMode::to_core),
                            },
                        ));
                    ...
                    self.on_collab_event(multi_agents::waiting_begin(
                        codex_protocol::protocol::CollabWaitingBeginEvent {
                            ...
                            receiver_agents: receiver_agents
                                .iter()
                                .filter_map(|agent| {
                                    Some(codex_protocol::protocol::CollabAgentRef {
                                        thread_id: app_server_collab_thread_id_to_core(
                                            &agent.thread_id,
                                        )?,
                    ...
                    self.on_collab_event(multi_agents::close_end(
                        codex_protocol::protocol::CollabCloseEndEvent {
                            ...
                            close_result: close_result
                                .map(codex_app_server_protocol::CollabCloseResult::to_core)
                                .unwrap_or(codex_protocol::protocol::CollabCloseResult::Closed),
                        },
                    ));
                }
            }
