...
fn plugin_status_label(plugin: &PluginSummary) -> &'static str {
    if plugin.installed {
        ...
        match plugin.install_policy {
            Some(PluginInstallPolicy::NotAvailable) | None => "Not installable",
            Some(PluginInstallPolicy::Available) => "Can be installed",
            Some(PluginInstallPolicy::InstalledByDefault) => "Available by default",
        }
    }
}
