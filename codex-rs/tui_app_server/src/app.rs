    ...
    fn event_survives_session_refresh(event: &ThreadBufferedEvent) -> bool {
        matches!(
            ...
            ThreadBufferedEvent::Request(_)
                | ThreadBufferedEvent::LegacyEvent(_)
...
impl App {
    ...
    async fn enqueue_thread_notification(
        ...
        {
            tracing::debug!("dropping stale notification for suppressed thread {thread_id}");
            return Ok(());
        }
        let (sender, store) = {
    ...
    async fn infer_session_for_thread_notification(
        ...
        self.observe_thread_parent_from_session_source(thread_id, &notification.thread.source);
    ...
    async fn enqueue_thread_request(
        ...
        if self.should_suppress_thread_events(thread_id) {
            tracing::debug!("dropping stale request for suppressed thread {thread_id}");
            return Ok(());
        }
    ...
    async fn enqueue_thread_legacy_warning(
        ...
        if self.should_suppress_thread_events(thread_id) {
            tracing::debug!("dropping stale legacy warning for suppressed thread {thread_id}");
            return Ok(());
        }
        let (sender, store) = {
            let channel = self.ensure_thread_channel(thread_id);
    ...
    async fn enqueue_thread_history_entry_response(
        ...
        if self.should_suppress_thread_events(thread_id) {
            tracing::debug!("dropping stale history response for suppressed thread {thread_id}");
            return Ok(());
        }
        let (sender, store) = {
            let channel = self.ensure_thread_channel(thread_id);
    ...
    async fn enqueue_thread_event(&mut self, thread_id: ThreadId, event: Event) -> Result<()> {
        if self.should_suppress_thread_events(thread_id) {
            tracing::debug!("dropping stale legacy event for suppressed thread {thread_id}");
            return Ok(());
        }
        let (sender, store) = {
            let channel = self.ensure_thread_channel(thread_id);
        ...
        let should_send = {
            ...
            guard
                .buffer
                .push_back(ThreadBufferedEvent::LegacyEvent(event.clone()));
            if guard.buffer.len() > guard.capacity
                && let Some(removed) = guard.buffer.pop_front()
                && let ThreadBufferedEvent::Request(request) = &removed
            {
        ...
        if should_send {
            match sender.try_send(ThreadBufferedEvent::LegacyEvent(event)) {
                Ok(()) => {}
                Err(TrySendError::Full(event)) => {
                    tokio::spawn(async move {
                        if let Err(err) = sender.send(event).await {
    ...
    async fn enqueue_thread_legacy_rollback(
        ...
        if self.should_suppress_thread_events(thread_id) {
            tracing::debug!("dropping stale rollback for suppressed thread {thread_id}");
            return Ok(());
        }
        let (sender, store) = {
            let channel = self.ensure_thread_channel(thread_id);
    ...
    async fn enqueue_primary_event(&mut self, event: Event) -> Result<()> {
        ...
        self.pending_primary_events
            .push_back(ThreadBufferedEvent::LegacyEvent(event));
        Ok(())
    }
    ...
    async fn enqueue_primary_thread_session(
        ...
        for pending_event in pending {
            match pending_event {
                ...
                ThreadBufferedEvent::LegacyEvent(event) => {
...
mod tests {
    ...
    -> Result<()> {
        ...
        app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
            config,
            auth_manager: app.chat_widget.auth_manager.clone(),
...
"#;
    ...
    async fn open_agent_picker_agent_shortcuts_move_selection_before_enter() -> Result<()> {
        ...
        assert!(
            !events.iter().any(|event| matches!(
                event,
                AppEvent::SelectAgentThread(selected_thread_id)
                    if *selected_thread_id == agent_thread_id
            )),
            "navigation shortcut should move the highlight without selecting, got {events:?}",
        );
        ...
        assert!(
            ...
            "expected SelectAgentThread({agent_thread_id}) after Enter, got {events:?}",
        );
        Ok(())
    }
    ...
    #[tokio::test]
    async fn enqueue_thread_notification_ignores_suppressed_descendant_updates() -> Result<()> {
        let mut app = make_test_app().await;
        let root_thread_id = ThreadId::new();
        let child_thread_id = ThreadId::new();
        ...
        app.enqueue_thread_notification(
            child_thread_id,
            turn_started_notification(child_thread_id, "turn-stale"),
        )
        .await?;
